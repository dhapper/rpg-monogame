...
{
    public static Entity CreateCrop(CropConfig cropConfig, EntityManager entityManager, AssetStore assets, (float x, float y) tilePos)
    {
        var crop = entityManager.CreateEntity();

        crop.AddComponent(new CropComponent { config = cropConfig.Clone() } );
